...
const GameBoard = () => {
  ...
  const getZoneColor = (row, col) => {
    if (row === 0) {
      if (col === 0) return 'bg-purple-600 border-purple-400'; // Strong Enchantment
      if (col >= 1 && col <= 6) return 'bg-yellow-700 border-yellow-500'; // Combat Eggs
      if (col === 7) return 'bg-gray-700 border-gray-500'; // Trash
    } else if (row === 1) {
      if (col === 0) return 'bg-indigo-600 border-indigo-400'; // Weak Enchantment
      if (col >= 1 && col <= 3) return 'bg-red-700 border-red-500'; // Traps
      if (col >= 4 && col <= 6) return 'bg-blue-700 border-blue-500'; // Spells
      if (col === 7) return 'bg-green-700 border-green-500'; // Library
    }
  ...
  const mockCard = {
    ...
    attack: 2500,
    defense: 2000,
    description: 'Un poderoso huevo dragón que puede destruir cualquier carta en el campo. Cuando es invocado, gana 500 puntos de ataque adicionales.',
    image: '🥚'
  };

  // Render a single 2x8 grid
    ...
    <div className="flex h-screen">
      ...
      <div className="flex-1 flex flex-col p-4 overflow-auto">
        ...
        <div className="mb-6 flex-1 flex flex-col">
          ...
          <div className="flex-1 flex items-center justify-center bg-gray-800/50 rounded-xl p-4">
            {renderGrid('opponent')}
          </div>
        </div>

        {/* Divider */}
        <div className="border-t-2 border-yellow-500 my-2"></div>
