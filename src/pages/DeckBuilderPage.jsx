...
import { Plus, Search, Edit, Save, X, Trash2 } from 'lucide-react';
...
import { loadDecks, saveSingleDeck } from '../utils/deckStorage';
...
export default function DeckBuilderPage() {
  ...
  useEffect(() => {
    if (user?.id) {
      const storedDecks = loadDecks(user.id);
  ...
  const handleSaveDeck = () => {
    if (!user?.id || !selectedDeck) {
      setSaveMessage({ type: 'error', text: 'No hay mazo seleccionado o usuario no autenticado' });
      setTimeout(() => setSaveMessage({ type: '', text: '' }), 3000);
      return;
    }

    const result = saveSingleDeck(user.id, selectedDeck, decks);
    if (result.success) {
      setDecks(result.decks);
      setSaveMessage({ type: 'success', text: `Mazo "${selectedDeck.name}" guardado correctamente` });
    } else {
      setSaveMessage({ type: 'error', text: result.error || 'Error al guardar el mazo' });
    }
    setTimeout(() => setSaveMessage({ type: '', text: '' }), 3000);
  };

  const startEditDeckName = (deck) => {
    setEditingDeckId(deck.id);
    setEditDeckName(deck.name);
  };

  const cancelEditDeckName = () => {
    setEditingDeckId(null);
    setEditDeckName('');
  };

  const confirmEditDeckName = () => {
    ...
    setDecks((prev) => {
      const updated = prev.map((deck) =>
        deck.id === editingDeckId ? { ...deck, name: trimmedName } : deck
      );
      // Persist only the name change to localStorage
      if (user?.id) {
        const key = `dueloDeHuevos:decks:${user.id}`;
        try {
          const stored = localStorage.getItem(key);
          if (stored) {
            const parsed = JSON.parse(stored);
            const storageUpdated = parsed.map((d) =>
              d.id === editingDeckId ? { ...d, name: trimmedName } : d
            );
            localStorage.setItem(key, JSON.stringify(storageUpdated));
          }
        } catch {
  ...
  const handleDeleteDeck = (deckId) => {
    if (!user?.id) return;
    if (decks.length <= 1) {
      setSaveMessage({ type: 'error', text: 'No se puede eliminar el último mazo' });
      setTimeout(() => setSaveMessage({ type: '', text: '' }), 3000);
      return;
    }
    ...
    setDecks((prev) => {
      ...
      // Persist to localStorage immediately on delete
      const key = `dueloDeHuevos:decks:${user.id}`;
      try {
      ...
      if (selectedDeckId === deckId) {
        setSelectedDeckId(updated[0]?.id ?? null);
      }
      return updated;
    });
  };
          ...
          <div className="grid h-full grid-rows-[36px_minmax(0,1fr)] gap-3 p-6">
            ...
            <div className="grid min-h-0 grid-cols-[300px_920px] gap-3">
              <div className="grid min-h-0 grid-rows-[106px_306px_300px_60px] gap-3">
                        ...
                        {editingDeckId === deck.id ? (
                          <div className="flex gap-1">
                            <input
                              ...
                              onKeyDown={(e) => {
                                if (e.key === 'Enter') confirmEditDeckName();
                                if (e.key === 'Escape') cancelEditDeckName();
                              }}
                              autoFocus
                              className="flex-1 rounded-lg border border-yellow-500 bg-gray-700 px-2 py-1 text-sm text-white focus:outline-none"
                              maxLength={50}
                            />
                            <button
                              onClick={confirmEditDeckName}
                              className="inline-flex h-8 items-center justify-center rounded-lg border border-green-500 bg-green-600 px-2 text-white hover:bg-green-700"
                              aria-label="Confirmar nombre"
                            >
                              <Save className="h-3.5 w-3.5" />
                            </button>
                            <button
                              onClick={cancelEditDeckName}
                              className="inline-flex h-8 items-center justify-center rounded-lg border border-red-500 bg-red-600 px-2 text-white hover:bg-red-700"
                          ...
                          <div className="flex items-center gap-1">
                            <button
                              ...
                              className={`flex-1 rounded-lg border px-3 py-2 text-left text-sm font-semibold leading-tight transition-colors ${
                            ...
                            <button
                              onClick={() => startEditDeckName(deck)}
                              className="inline-flex h-8 items-center justify-center rounded-lg border border-gray-600 bg-gray-700 text-white hover:bg-gray-600"
                ...
                {saveMessage.type && (
                  <div
                    className={`mb-2 text-sm px-3 py-2 rounded-lg ${
                ...
                <section className="h-full overflow-hidden rounded-xl border border-yellow-500 bg-gray-800">
                  <div className="grid h-1/2 grid-cols-2">
                    <button
                      ...
                      className="border-r border-gray-700 bg-gray-700 text-sm text-white hover:bg-gray-600 flex items-center justify-center gap-1"
                    >
                      <Save className="h-3.5 w-3.5" />
                      Guardar
                    </button>
                  ...
                  <div className="grid h-1/2 grid-cols-2 border-t border-gray-700">
                    <button
                      ...
                      className="border-r border-gray-700 bg-gray-700 text-sm text-white hover:bg-gray-600 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-1"
                    >
